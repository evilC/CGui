...
class _CGui extends _CGuiBase {
	...
	__New(parent := 0, options := 0, aParams*){
		...
		this._MessageHandlerFunc := this._MessageHandler.Bind(this)
		...
		;MsgBox % this._DLL_GetScrollInfo(SB_HORZ).Size()
	...
	Destroy(){
		...
		for msg, value in _CGui
			if InStr(msg,"`r")=1
				value.Remove(this._hwnd)
	...
	*/
	...
	_BlankScrollInfo(){
		return new _Struct(WinStructs.SCROLLINFO,{cbSize:sizeof(WinStructs.SCROLLINFO)})
	}
	...
	*/
	...
	_MessageHandler(wParam, lParam, msg, hwnd){
		...
		if _CGui["`r" msg].HasKey(hwnd+0)
			%_CGui["`r" msg][hwnd+0]%(wParam, lParam, msg, hwnd)
	}
	...
	_OnWheel(wParam, lParam, msg, hwnd){
		...
		if (!ObjHasKey(_CGui["`r" msg], hcurrent)){
			return
		}
		...
		%_CGui["`r" msg][hcurrent+0]%(sb, 0, msg, hcurrent)
		;return 0
	}
